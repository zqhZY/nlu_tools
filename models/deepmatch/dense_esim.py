...
class DESIM(BaseModel):
    def __init__(self, config):
        super(DESIM, self).__init__(config)
        self.build_model()
        self.init_saver()
    ...
    def embedding_block(self):
        ...
        return self.embed1, self.embed2
    ...
    def bilstm_encoding_block(self, embed1, embed2, scope):
        ...
        with tf.variable_scope(scope):
            ...
            if self.config.using_actual_len:
                outputs_x1, final_states_x1 = _bilstm_block(embed1, self.config.hidden_size, 'bilstm', seq_len=self.x1_mask)
                outputs_x2, final_states_x2 = _bilstm_block(embed2, self.config.hidden_size, 'bilstm', seq_len=self.x2_mask, reuse=True)
            else:
                outputs_x1, final_states_x1 = _bilstm_block(embed1, self.config.hidden_size, 'bilstm',self.config.dropout)
                outputs_x2, final_states_x2 = _bilstm_block(embed2, self.config.hidden_size, 'bilstm', self.config.dropout, reuse=True)
    ...
    def essemble_block(self, layers):
	    ...
	    # scale the weighted sum by gamma
            ...
            normed_weights = tf.split(tf.nn.softmax(W + 1.0 / n_layers), n_layers)
    ...
    def build_model(self):
        ...
        # build embeds
        embed1, embed2 = self.embedding_block()
        f_layer1 = self.interact_block(embed1, embed2, "feature_layer1")

        # build bilstm encoding block
        x1_bar, x2_bar = self.bilstm_encoding_block(embed1, embed2, "encoding_block")
        f_layer2 = self.interact_block(x1_bar, x2_bar, "feature_layer2")
        
	# build local attention inference block
        # [batch, seq_len, 4*hidden_size]
	# need embedding infomation???
        m_x1, m_x2 = self.local_infer_block(x1_bar, x2_bar, "local_infer_block")

        # mixed bilstm emcode block
        x1_out, x2_out = self.bilstm_encoding_block(m_x1, m_x2, "composition_block")
        f_layer3 = self.interact_block(x1_out, x2_out, "feature_layer3")
        
	# label essemble layer
        ...
        with tf.name_scope("loss"):
            if self.config.loss == "mse":
                self.cost = tf.reduce_mean(tf.losses.mean_squared_error(labels=self.y, predictions=self.pred))
            else:
