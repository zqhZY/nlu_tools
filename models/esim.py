...
class ESIM(BaseModel):
    ...
    def input_encoding_block(self, scope):
        ...
        with tf.variable_scope(scope):
            # a_bar = BiLSTM(a, i) (1)
            # b_bar = BiLSTM(b, i) (2)
            #outputs_x1, final_states_x1 = _bilstm_block(self.embed1, self.config.hidden_size, 'bilstm', self.x1_mask)
            #outputs_x2, final_states_x2 = _bilstm_block(self.embed2, self.config.hidden_size, 'bilstm', self.x1_mask, reuse=True)
            outputs_x1, final_states_x1 = _bilstm_block(self.embed1, self.config.hidden_size, 'bilstm')
            outputs_x2, final_states_x2 = _bilstm_block(self.embed2, self.config.hidden_size, 'bilstm', reuse=True)
    ...
    def composition_block(self, m_a, m_b, hidden_size, scope):
        ...
        """
        with tf.variable_scope(scope):
            outputV_a, finalStateV_a = _bilstm_block(m_a, hidden_size, 'biLSTM')
            outputV_b, finalStateV_b = _bilstm_block(m_b, hidden_size, 'biLSTM', reuse=True)
            ...
            print_shape('v', v)
            y_hat = _feedforward_block(v, self.config.dense_size, self.config.n_classes, 'feed_forward')
            return y_hat
